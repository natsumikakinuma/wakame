...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $stmt = $pdo->prepare('INSERT INTO profiles (user_id, name, age, gender, hair_color, hair_texture, straightening, straightening_date, color, color_date, black_dye, black_dye_date, hairstyle) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    if ($stmt->execute([$user_id, $name, $age, $gender, $hair_color, $hair_texture, $straightening, $straightening_date, $color, $color_date, $black_dye, $black_dye_date, $hairstyle_path])) {
        echo 'プロフィール保存成功<br>';
        echo 'リダイレクト中...<br>';
        header('Location: home.php');
        exit;
    } else {
        echo 'プロフィールの保存に失敗しました。';
    }
} else {
